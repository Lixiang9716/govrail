...
一个面向 agent 驱动开发的治理平面：让 coding agent 快速并行工作，同时由机器——而非人的警惕——守住质量线。治理机制（门禁、笔记、回执、配对）与语言无关；代码事实层跑在今天随包分发的 10 种 tree-sitter grammar 上（c、cpp、go、java、javascript、kotlin、python、rust、swift、typescript——更多语言用同样方式加 `.gov/checks/<lang>.json` 规则；check 门禁只判有规则的语言，而 `gov parse`/`gov stats` 读全部已装语法，Swift 与 Kotlin 也在内）。运行时是 Python 3（>= 3.10）——全部由 `pip install govrail` 安装，无需其他工具。
