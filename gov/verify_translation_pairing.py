...
def _render_record(fields: dict[str, str]) -> str:
    ...
    cannot perturb the side hashes it pins. A side with no commit yet
    carries the literal ``untracked`` (#345): the same sentinel the CLI
    prints — a bare empty scalar read like corruption and was not
    greppable."""
    return (
        "# Pairing record: the state at the moment this pair was last\n"
        "# confirmed consistent. Regenerated by `gov verify pairing "
        "--write`.\n"
        ...
        "# compares only the en/zh hashes. 'untracked' = no commit yet.\n"
        "# last_confirmed is the UTC ISO-8601 instant of the confirmation\n"
        "# (datetime.isoformat(), +00:00).\n"
        "pair:\n"
        f"  en: {fields['en']}\n"
        f"  zh: {fields['zh']}\n"
        f"counterpart: {fields['counterpart']}\n"
        f"last_confirmed: {fields['last_confirmed']}\n"
        f"en_commit: {fields['en_commit'] or 'untracked'}\n"
