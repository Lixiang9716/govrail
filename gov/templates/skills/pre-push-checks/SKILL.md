...
4. Run the chosen mode, the `--base` selection, or individual commands. If there are unstaged or untracked files, either include them or exclude them from your reasoning — never assume they are part of the diff.
   ...
   A push scopes to what the push CARRIES: the hook resolves a new branch's fork point, declares the range in `GOV_CHANGE_BASE` (which change-scoped tools honor), and a ranged base lists committed content only — untracked files are not part of it. If a gate still goes red on a file the push does not carry, make the tree clean or push from a detached worktree (`git worktree add --detach /tmp/x <sha>`); `--no-verify` is the last resort, not the fix.
