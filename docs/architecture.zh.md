...
`gov` 包本身就是一个模块化单体，这是承诺而非巧合——五条契约撑着这 1.6 万行持续生长：

1. **模块化单体。** 一个平铺包、一个关注点一个模块、三层无例外：叶工具（`atomicio`、`gitutil`、`pathmatch`、`root`、`lockfile`、`version`、`anchor`、`merge`，以及零依赖的注册表 `commands`、`decisions`、`whatsnew`）→ 领域模块（一个命令族一个）→ 顶层的分发器（`cli.py`），只被声明的入口导入。`import-layers` 门审判这张图——叶保持是叶、谁也不许向上够分发器、导入**期**的图无环。函数级惰性导入是既定的解耦手段（方向规则对它照常生效）；它们被计入门的汇总行，永不隐形。
2. **注册表驱动的表面。** 每一份机器消费的词汇表都是"一个家的一张表"：`gov/commands.py` 持有命令面板（`--help` 渲染它、audit-notes 把它当已知命令集、退出码契约测试拿它当母名单）；self-test 案例经 `@case` 装饰器注册——案例只需定义，永不需要再手工登记。新增命令只碰注册表和 `govrail` 技能的阶段表，别无其他（rule 9）。
3. **函数式核心，命令式外壳。** 解析、校验、选择、哈希都是输入的纯函数（`load_config`、`_select_by_paths`、规范化序列化）；git、子进程、写盘只住在 `main()` 与外壳层。可测性来自纯度，不来自注入框架。
