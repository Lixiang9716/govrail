...
1. **模块化单体。** 一个平铺包、一个关注点一个模块、三层无例外：叶工具（确切集合声明在 `scripts/import-layers.json`——这份清单的家是门禁配置，不是这段散文）→ 领域模块（一个命令族一个）→ 顶层的分发器（`cli.py`），只被声明的入口导入。`import-layers` 门审判这张图——叶保持是叶、谁也不许向上够分发器、导入**期**的图无环。函数级惰性导入是既定的解耦手段（方向规则对它照常生效）；它们被计入门的汇总行，永不隐形。
