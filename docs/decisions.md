...
- **修订（#265，栈即接口）**：解析栈对采用者是一等接口——govrail 随包分发 tree-sitter 核心 + 全部语法包，项目门禁可以 `import tree_sitter_*` 直接消费；**tree-sitter 的版本由 govrail 管理**，项目不得在 CI/requirements 里单独钉它（同环境降级核心会把语法 ABI 打回不兼容——dsh-mobile 实测）。消费者原语是 `gov parse`（逐文件函数 span/行数/嵌套深度，--json 恰一个值）与 `gov stats` 的聚合事实；自定义 size/复杂度门 = 声明限额 + 读这些数字。

## D55 — check 门进默认门集：默认 DAG 从第一天就长着看产品代码的眼睛

- **问题**：外部审查反复点名的同一结构性项（"默认 gate 不查产品代码"）：init 注入的默认门禁集只看治理面（notes/pairing/plane/task…），产品代码本身没有任何默认门禁真正检查——`conflict-markers` 无 paths 是唯一扫到产品文件的内容门，但它只认冲突标记这一种字面证据；一个 src/ 里满是未钉 encoding 的 text-mode spawn 的项目，默认 DAG 照样全绿。而 D54"后续（事件到了再建）"里建的检查引擎（`gov check`：tree-sitter 语法类规则，shipped 8 语言 + `.gov/checks/` 项目扩展）落地后从未进任何默认门集——连 dogfood 仓库自己的 gates.json 都没跑它，"命令能查的进命令"（rule 1）在自己家里没兑现。
- **选项**：(a) 维持现状，产品检查全走 `--preset`（D28 口径）；(b) 模板默认加 `check` 门：无 paths、全树扫描、语言规则按扩展名自选、error 级拦截 warning 只报告；(c) 默认加但 `allowFailure: true`（advisory）
- **状态**：已决
...
- **修订二（rule 9，skills 路由成为必须点名的家）**：机器接口纪律的第三个家——`govrail` 路由 skill 的阶段表。`gov update`（D58）发布时漏更新了它：命令能跑、--help 诚实、契约有钉，唯独 agent 枚举的面上不可见。已随 rule 9 写入 `.gov/rules.md`：新增用户可见命令时，help/flag registry、skills 阶段表、退出码契约登记表三处在同一 PR 更新。
...
## D60 — agent 钩子多平台：一个核心，每个平台一种方言

- **问题**：D59 只接了 Claude Code——安装面只有 `.claude/settings.json`，处理器只会 `hookSpecificOutput` 一种输出方言。Codex CLI、GitHub Copilot CLI、Gemini CLI 相继落地钩子系统后，以这些平台为主力工具的采用者毫无存在感：无 pre-tool-use 拒绝、无上下文注入。官方文档核实了单模板不可行的原因：各家在"命令 + stdin JSON"上共识，在其余一切上分裂——配置文件（`.codex/hooks.json` / `.github/hooks/*.json` / `.gemini/settings.json`）、事件名（Codex 沿用 Claude 的 PascalCase；Copilot 用 camelCase；Gemini 给两个事件改名）、拒绝机制（Copilot 读顶层 `permissionDecision`；Gemini 文档化的阻断是 exit 2 + stderr 理由）、信任模型（Codex 对未信任的项目钩子静默跳过，须人工 `/hooks` 信任）。
- **选项**：(a) 维持 Claude 单平台；(b) `agent-hooks --dialect <平台>` 输出方言层 + `init --platforms <列表>` 每平台一个单源模板；(c) 追随 Copilot 的 Claude 兼容模式（它也读 `.claude/settings.json`），不落新文件
- **状态**：已决
- **决定**：(b)。处理器核心共用，方言只管输出契约：claude/codex 的 deny 走 `hookSpecificOutput.permissionDecision`（Codex 文档化形状与 Claude 逐字节同形），codex 的上下文走纯文本 stdout（其 developer-context 通道），copilot 的 deny/上下文走顶层 `permissionDecision`/`additionalContext`，gemini 的 deny 走 exit 2 + stderr 理由（其 "System Block"，仍在 0/2 退出码契约内）、上下文的 hookEventName 按 Gemini 的开火名拼写（`BeforeAgent` 而非 `UserPromptSubmit`）；载荷解析兼容 `tool_input` 与 Copilot 的 `toolArgs` 两种命令键拼写。`gov init --platforms`（fresh 或 retrofit 皆可，`all` 合法，未知名 exit 2）安装每平台一份模板——`codex-hooks.json`→`.codex/hooks.json`、`copilot-hooks.json`→`.github/hooks/govrail.json`、`gemini-settings.json`→`.gemini/settings.json`——create-if-missing、已存在即点名跳过，D59 的四读者性质原样保持（init 写入、`_inventory` 漂移分类、uninstall 定制检查、`_template_for` 逆向映射读同一份字节），新平台配置零新机制即精确可逆；manifest 以合并式 `platforms[]` 记录选择。处处显式优于推断：方言由 init 写进各平台钩子命令、不从载荷猜；显式 `--platforms` 取代 claude-only 默认（今日 `--hooks` retrofit 行为不变），裸 init 不装任何平台配置、与从前逐字节一致；Codex 的信任步骤在安装时点名——不被信任的钩子是静默空转。
- **被否**：(a) 单平台——采用者主力平台的会话中途拒绝与注入是真实缺口；(c) 兼容模式——其 deny 决定的处理无文档，拒绝可能静默失效（fail-open），一等公民文件 + 文档化的顶层键才是可检查的承诺；**从载荷形状推断方言**——两个平台今天同形明天分叉，显式旗标 + init 写死让漂移响亮失败（rule 5）；**所有平台配置随每次 init 全装**——不用的平台文件成为 `_inventory` 漂移报告的噪音，upgrade 报告点名可装平台、`--platforms` 保持刻意动作；**同批支持 Cursor/Windsurf/OpenCode**——它们需要事件扇入（Cursor 把工具拆成 `beforeShellExecution`/`beforeFileEdit`/…）或进程内插件 shim（OpenCode），是第二步设计而非一个模板，列为后续批次。
...
## D62 — D62 — 惊讶台账：预期≠现实入账，第三次重复升级为流程改进

- **问题**：平面记录失败（notes/decisions/postmortems）却不记录"惊讶"——现实与预期不一致的那个瞬间。它只存在于对话里，随即蒸发；同一类惊讶可以发生五次而无人计数，每次都被当作一次性倒霉单独修掉，而真正该改的是流程。既有体裁全是回望式或决策式：postmortem 门槛高（隐蔽+系统性+代价高），notes 记决定不记误期。缺的是轻粒度的前瞻遥测：记录、计数、到阈值强制升级。
- **选项**：(a) 不做机制，靠人脑记忆三次重复；(b) 惊讶台账 + 记录时相似检索 + 第三击门禁升级：`gov surprise record/list`（tracked append-only `.gov/surprises.jsonl`，签名分组），`surprises` 门（blocking，自吃专用）要求任一签名 ≥3 条时必须存在引用 `surprise:<sig>` 的 process note，rule 11 入宪；(c) 只加 prose 台账 + README 惯例
- **状态**：已决
